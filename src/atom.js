...
export class AtomViewer {
  ...
  loadElement(element, options = {}){
    ...
    // create nucleus as cluster of small spheres
    const nucleus = new THREE.Group();
    const N = Math.max(1, Math.round((element.protons + (element.neutrons||element.number))/6));
    ...
    for(let i=0;i<element.protons;i++){
      ...
      s.position.set((Math.random()-0.5)*N,(Math.random()-0.5)*N,(Math.random()-0.5)*N);
      nucleus.add(s);
    }
  ...
  toggleShells(visible){
    this.shells.forEach(s=> s.mesh.visible = visible);
  }

  setRealisticScale(enabled){
    if(!this.nucleus) return;
    this.nucleus.scale.setScalar(enabled? 0.2 : 1.0);
  }
  ...
  update(dt){
    ...
    for(const e of this.electrons){
      ...
      const a = e.angle;
      const r = e.radius;
      // simple circular orbit in XY plane; add small inclination per shell
